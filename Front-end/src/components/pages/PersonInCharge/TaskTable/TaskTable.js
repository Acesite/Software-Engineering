    ...
    <div className="bg-white p-6 rounded shadow-md overflow-x-auto">
      <div className="flex items-center justify-between mb-4">
        {filteredTasks.length > 0 && (
          <h1 className="text-xl font-semibold text-black-500">Assigned Task for Students</h1>
        )}
        <div>
          <select
            value={filter}
      ...
      {filteredTasks.length === 0 ? (
        <p className="text-center mb-10">No tasks available for the selected filter.</p>
      ) : (
            ...
            <tbody>
              {currentTasks.map((task, index) => (
                <tr key={index} className="text-center">
                  ...
                  <td className="py-2 px-4 border-b border-r text-sm align-middle">
