...
const ViolationSelectModal = ({ violationsData, isOpen, closeModal, onSelectViolation }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");
  const violationsPerPage = 5; // Number of violations per page
  ...
  const filteredViolations = violationsData.filter(
    (violation) => violation.violation.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[800px] max-h-[90vh] overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-semibold text-gray-800">Select a Violation</h3>
            <div className="text-sm text-gray-600">
              Page {currentPage} of {totalPages} | Total Items: {filteredViolations.length}
            </div>
          </div>
        ...
        <div className="overflow-y-auto max-h-[50vh]">
          <table className="min-w-full table-fixed">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                <th className="w-1/3 px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-b">
                  Violation
                </th>
                <th className="w-1/3 px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-b">
                  Duty Hours
...
export default ViolationSelectModal;
