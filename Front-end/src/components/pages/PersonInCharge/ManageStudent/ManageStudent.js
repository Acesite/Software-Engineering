...
import SidebarNavigation from '../PersonSidebar/PersonSidebar';
...
const ManageStudent = () => {
  ...
  const [violations, setViolations] = useState([]); // New state for violations
  ...
  useEffect(() => {
    fetchTasks();
    fetchStudents();
    fetchViolations(); // Fetch violations
  }, []);
  ...
  const fetchViolations = async () => {
    try {
      const response = await axios.get('http://localhost:3001/violations'); // Adjust the URL if needed
    ...
    <div className="flex bg-[#f3f3f3]">
      ...
      <div className="w-full ml-64 p-6">
        <TaskForm 
          ...
          violations={violations} // Pass violations to TaskForm
        />
        <TaskTable 
