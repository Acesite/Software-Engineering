...
function PersonInCharge() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      axios
        .get('http://localhost:3001/auth/person/dashboard-data', {
          headers: {
        ...
        .then((response) => {
          ...
          if (!toastShown.current) {
            toast.success('Successfully logged in as Person in Charge!');
    ...
    <div className="flex min-h-screen ">
      ...
      <div className="flex-1 bg-gray-100">
        ...
        <div className="sticky top-0  flex items-center justify-between p-6 bg-[#f3f3f3] ">
          <h1 className="text-3xl font-bold text-black-500">Person in Charge Dashboard</h1>
          <Clock />
        </div>
